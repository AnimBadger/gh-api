...
function trackableMockFetch(status: number, body: Record<string, unknown>): () => void {
  const original = globalThis.fetch;
  fetchCalls = [];
...
test.describe("Otp.sendOtp", () => {
  test("returns success when API returns code 1000", async () => {
    const restore = mockFetch(200, { code: "1000", message: "OTP sent", ussd_code: "*928*01#" });
    const result = await otp.sendOtp({ number: "233541232346", senderId: "Test", expiry: 5 });
    restore();
  ...
  test("sends the correct request body", async () => {
    ...
    const body = JSON.parse(fetchCalls[0]?.options.body as string) as Record<string, unknown>;
...
test.describe("Otp.verifyOtp", () => {
  test("returns success when API returns code 1100", async () => {
    const restore = mockFetch(200, { code: "1100", message: "Verification successful" });
    const result = await otp.verifyOtp({ code: "123456", number: "233541232346" });
    restore();
  ...
  test("returns failure when API returns non-1100 code", async () => {
    ...
    const result = await otp.verifyOtp({ code: "000000", number: "233541232346" });
    restore();
